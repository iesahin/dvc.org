...
```usage
usage: dvc remote list [-h] [--global | --system | --repo | --local] [-q | -v]
```
...
`--repo`, and `--local` can be used to tell the command to read from only that
location.
...
- `--repo` - read remote configuration from the project's config
  (`.dvc/config`).
