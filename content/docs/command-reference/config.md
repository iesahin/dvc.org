...
```usage
usage: dvc config [-h] [--global | --system | --repo | --local] [-q | -v] [-u]
                  [-l] [name] [value]
...
options `--system`, `--global`, `--repo`, and `--local` can be used to tell the
...
write to that location (you can say `--repo` but that is the default). The
